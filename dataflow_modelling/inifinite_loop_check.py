...
def is_infinite_loop(project,initial_state,addr,isthumb):
    ...
    for i in range(5):
        ...
        if bbl_addr == None :
            bbl_addr = active_state.solver.eval_one(active_state.regs.pc) & 0xfffffffe
        if (active_state.solver.eval_one(active_state.regs.pc) & 0xfffffffe)  == addr:
            print("------------------",hex(addr),hex(bbl_addr)) 
...
def find_all_infinite_loop(project, initial_state,global_cfg):
    ...
    for i in range(0,memseg.size,ins_size):
        ...
        for insn in md.disasm(instruction_bytes, memseg.start + i):
            if "b" == insn.mnemonic:
                is_loop,bbl_addr = is_infinite_loop(project,initial_state,memseg.start + i,ins_size == 4)
                if is_loop:
                    all_loops.append(bbl_addr)
    ins_size = 2
    for i in range(0,memseg.size,ins_size):
        disassembly_block = project.factory.block(memseg.start + i, size=ins_size)
        instruction_bytes = disassembly_block.bytes
