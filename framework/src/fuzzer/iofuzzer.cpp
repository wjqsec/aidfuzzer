...
{
  bool ret;
  bool queue_state_updated = false;
...
void fuzz_runonce(FuzzState *state)
{
  ...
  for(queue_entry *q : *state->entries)
  {
    ...
    {
      printf("queue %x not found stream %x\n",q->cksum,exit_info.stream_info.exit_stream_id);
    }
