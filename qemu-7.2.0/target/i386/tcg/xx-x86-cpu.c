...
x86_cpu_do_interrupt_cb x86_cpu_do_interrupt_func;

typedef void (*x86_cpu_exec_interrupt_cb)(); 
x86_cpu_exec_interrupt_cb x86_cpu_exec_interrupt_func;

typedef void (*x86_cpu_do_unaligned_access_cb)(); 
x86_cpu_do_unaligned_access_cb x86_cpu_do_unaligned_access_func;

void register_x86_cpu_do_interrupt_hook(x86_cpu_do_interrupt_cb cb)
{
    x86_cpu_do_interrupt_func = cb;
}
void xx_x86_cpu_do_interrupt(CPUState *cs)
{
    if(x86_cpu_do_interrupt_func)
        x86_cpu_do_interrupt_func();
    x86_cpu_do_interrupt(cs);
}

void register_x86_cpu_exec_interrupt_hook(x86_cpu_exec_interrupt_cb cb)
{
    x86_cpu_exec_interrupt_func = cb;
}
bool xx_x86_cpu_exec_interrupt(CPUState *cs, int interrupt_request)
{
    if(x86_cpu_exec_interrupt_func)
        x86_cpu_exec_interrupt_func();
    return x86_cpu_exec_interrupt(cs,interrupt_request);
}

void register_x86_cpu_do_unaligned_access_hook(x86_cpu_do_unaligned_access_cb cb)
{
    x86_cpu_do_unaligned_access_func = cb;
}
G_NORETURN void xx_x86_cpu_do_unaligned_access(CPUState *cs, vaddr vaddr,
                                            MMUAccessType access_type,
                                            int mmu_idx, uintptr_t retaddr)
{
    if(x86_cpu_do_unaligned_access_func)
        x86_cpu_do_unaligned_access_func();
    return x86_cpu_do_unaligned_access(cs,vaddr,access_type,mmu_idx,retaddr);
}
static const struct TCGCPUOps x86_tcg_ops = {
    ...
    .do_interrupt = xx_x86_cpu_do_interrupt,
    .cpu_exec_interrupt = xx_x86_cpu_exec_interrupt,
    .do_unaligned_access = xx_x86_cpu_do_unaligned_access,
